...
router.get('/', [
  ...
  query('sortBy').optional().isIn(['title', 'author', 'publication_date']),
  ...
  try {
    let queryStr = 'SELECT id, title, author, cover_image, publication_date FROM books';
    const queryParams = [];
...
router.post('/', auth, authorize(['admin']), upload.single('coverImage'), [
  ...
  try {
    ...
    const { rows } = await pool.query(
      'INSERT INTO books (title, author, cover_image, publication_date) VALUES ($1, $2, $3, $4) RETURNING id, title, author, cover_image, publication_date',
      [title, author, coverImageUrl, publication_date]
    );
